...
def client_cmd(cmd):
    p = sh("docker", "compose", "exec", "-T", "client", "python", "/app/internal_probe.py", cmd)
...
SOURCE_FILES = [
    ROOT.parent.parent / "manifests" / "A-6.yaml", ROOT / "docker-compose.yml",
    ROOT / "client" / "client.py", ROOT / "resolver" / "resolver.py",
    ROOT / "target" / "target.py",
]
...
def wait_up():
    ...
    while time.time() < deadline:
        try:
            if get("http://127.0.0.1:18172/health").get("ok"):
                return
        except Exception:
            time.sleep(0.5)
...
def one_run(n, level):
    ...
    client_cmd("RESET")
    baseline_t = client_cmd("CONNECT resolver-t zone-t")
    client_cmd("RESET")
    baseline_c = client_cmd("CONNECT resolver-c zone-c")
    ...
    client_cmd("RESET")
    withdraw = client_cmd("WITHDRAW resolver-t")
    access = client_cmd("ACCESS_STATE")

    post_t = client_cmd("CONNECT resolver-t zone-t")
    control_c = client_cmd("CONNECT resolver-c zone-c")
    ...
    record = {
        "test": "A-6", "harness_version": "0.2.0", "run": n, "level": level,
        "timestamp_utc": datetime.now(timezone.utc).isoformat(),
        "baseline_T": bt, "baseline_C": bc,
        "post_prediction_T": [1,1,0,0], "post_T": pt,
    ...
    print(f"A-6 {level} run {n}: baseline T={tuple(bt)} C={tuple(bc)} | post T={tuple(pt)} C(control)={tuple(cc)} => {outcome}")
    return record

def dry_run():
    print("Dry run only: A-6 WILL NOT BE EXECUTED.\n")
    ...
    print("\nHarness is ready. This is the paper's contested classification --")
    print("see manifests/A-6.yaml 'classification_note' before treating results")
    print("as settling whether name resolution belongs on the Authority row.")
...
def main():
    ...
    print("WARNING: this executes preregistered H4 configuration A-6.")
