...
SOURCE_FILES = [
    ROOT.parent.parent / "manifests" / "C-4c.yaml", ROOT / "docker-compose.yml",
    ROOT / "resolver-d1" / "resolver.py", ROOT / "resolver-d2" / "resolver.py",
    ROOT / "gateway-h" / "resolver.py", ROOT / "target" / "target.py", ROOT / "client" / "send.py",
]
...
def wait_up():
    ...
    while time.time() < deadline:
        try:
            if get("http://127.0.0.1:18360/health").get("ok") and get("http://127.0.0.1:18361/health").get("ok") \
...
def sub_ablation(n, level, sub_id, stop_service, prediction):
    ...
    d1_ok = probe_d1_post.get("status") == 200 and probe_d1_post.get("body", {}).get("marker_seen") is True
    d2_ok = probe_d2_post.get("status") == 200 and probe_d2_post.get("body", {}).get("marker_seen") is True
    X_post = int(d1_ok or d2_ok)
    ...
    record = {
        "test": "C-4c", "sub": sub_id, "harness_version": "0.2.0", "run": n, "level": level,
        ...
        "resolver_d1_probe_ok": d1_ok, "resolver_d2_probe_ok": d2_ok,
...
def dry_run():
    print("Dry run only: C-4c WILL NOT BE EXECUTED.\n")
    ...
    print("Harness is ready. Runs C-4c-i (stop gateway-h) and C-4c-ii (stop resolver-d1) from separate fresh builds.")
...
def main():
    ...
    print("WARNING: this executes preregistered H4 configuration C-4c (two sub-ablations, each from a fresh build).")
    print(f"Level: {level}"); print(f"Runs (each): {runs}")
    if input("Type EXECUTE to continue: ") != "EXECUTE": print("Cancelled."); return
    d = RESULTS / level
    existing_i = [int(p.name.split("-")[1]) for p in d.glob("run-*-i")] if d.exists() else []
    start = max(existing_i) + 1 if existing_i else 1
    recs = []
    for n in range(start, start + runs):
        recs.append(sub_ablation(n, level, "i", "gateway-h", [1, 0, 1, 0]))
        recs.append(sub_ablation(n, level, "ii", "resolver-d1", [1, 1, 1, 1]))
    outcomes = {}
